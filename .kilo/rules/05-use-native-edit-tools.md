...
You are running inside a Kilo code workspace environment with native tools enabled. You **MUST NOT** use `<tool_code>`, `<tool_call>`, or pythonic block calls like `read_file(filepath="...")`.
...
* Rely exclusively on the standard tool-calling API payload structure provided in your system context.
* You **MUST** invoke your native `single_find_and_replace` tool capability to apply changes directly.
